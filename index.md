...
③ 프로그래밍
1. 각 버튼마다 화면 변경
2. 첫 화면 게임시작 버튼을 눌러 스토리 창에 왔을 때 스토리 창 넘기기 (스페이스바)
 2-1. 엔딩(캐릭터가 데이트하는 모습)의 스토리 창 넘기기
3. 스토리창이 서서히 검은 창으로 변하며 메인화면 등장 (페이트 효과주기)
 3-1. 외출에서 각 버튼(산, 헬스장, 공원, 오락실, 먹거리)을 눌렀을 때, 행동이 끝났을 때 
 3-2. 엔딩(캐릭터가 데이트하는 모습, 과로한 모습, 행복한 모습)이 뜰 때
 3-3. 외출 2번을 했을 때 메인 화면으로 돌아올 때 
4. 외출에서 각 버튼(산, 헬스장, 공원, 오락실, 먹거리)의 몸무게, 스트레스, 행복지수 감소, 증가 
5. 외출 2번을 마친 후, 다음 날로 변경 (D-7 => D-6)
6. 외출 2번을 마친 후, 다음 날이 왔을 때 스트레스 –10% 
7. 엔딩 
 7-1. D-7 이라는 시간이 다 흘렀을 때
 7-2. 스트레스 100% 됐을 때
 7-3. 행복지수 100% 됐을 때
